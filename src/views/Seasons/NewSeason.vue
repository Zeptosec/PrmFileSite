    ...
    <main>
        <div class="page" v-if="pageData != null">
            ...
            <OrderTable v-if="pageData.length > 0" :files="pageData" :index-of="indexOf"
                @changeOrder="w => changeOrder(w.fileid)" @sort="w => sort(w)" />
            ...
            <div class="form">
                ...
                <button :disabled="isCreating" @click="Create">Create</button>
...
async function Create() {
    ...
    if (!createError.value) {
        const { error } = await supabase
            .from('Seasons')
            .insert({ name: seasonName.value, episodeIds: order.value, userid: props.theUser.id })
        if (error) {
